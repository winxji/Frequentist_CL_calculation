...
public:
  TClass_CLs() {
    ...
    dm2_low = 0.01; //0.61;/// hhack, ue = 0.15, ee = 0.65
    dm2_hgh = 94;
    dm2_step = 0.05;
  }
...
{  
  ...
  for(int idx=0; idx<vc_size; idx++) {
    ...
    //if(idm2==38 && itheta==46) cout<<" ---> check "<< dchi2 <<endl;
  }
  ...
  if( 0 ) {
    ...
    //TCanvas *canv_h2d_space = new TCanvas("canv_h2d_space", "", 900, 800);
    h2d_space->SetXTitle("log10(sin^{2}2#theta)");
    h2d_space->SetYTitle("log10(#Deltam^{2}_{41})");
    h2d_space->SetTitle("z-axis: confidence level");
    h2d_space->SaveAs("h2d_space_test.root");
  }
  ...
  //h2d_space->SaveAs("h2_CL_map.root");//xyz
  ...
  if( 1 ) {// draw
    ...
    roostr = "h2d_data";
    TH2D *h2d_data = new TH2D(roostr, "", 10, 1e-4, 1, 10, 0.01, 100);
    h2d_data->Draw();
...
void TClass_CLs::func_sensitivity()
{
  ...
  if( 1 ) {
    ...
    roostr = "h2d_sens";
    TH2D *h2d_sens = new TH2D(roostr, "", 10, 1e-4, 1, 10, 0.01, 100);
    h2d_sens->Draw();
...
double TClass_CLs::func_quantiles(double qq, double dm2_val, bool flag_plot)
{
  ...
  for(int idx=0; idx<vc_size; idx++) {
    ...
    //cout<<" ---> check "<<dm2_edge1<<"\t"<<dm2_edge2<<endl;
...
void TClass_CLs::plot_CLs_toys()
{
  ...
  roostr = "h2d_CLs_toys";
  TH2D *h2d_CLs_toys = new TH2D(roostr, "", 10, 1e-4, 1, 10, 0.01, 100);
  h2d_CLs_toys->Draw();
  ...
  for(int idx=0; idx<vc_size; idx++) {
    ...
    //vec_graph_95_toys.at(10)->Draw("same l");
    //vec_graph_95_toys.at(10)->SetLineColor(kGray+1);
...
void TClass_CLs::func_get_contours(TH2D *h2_CL, TGraph *gh_CL[3], int index)
{
  ...
  /////// test 
  /////// test 
  // TFile *userfile = new TFile("total_data_grids_dm2_t14_fit_t24.root", "read");
  // TTree *usertree = (TTree*)userfile->Get("tree");
  ...
  // // Set branch addresses and branch pointers
  // usertree->SetBranchAddress("usr_idm2", &usr_idm2, &b_usr_idm2);
  // usertree->SetBranchAddress("usr_it14", &usr_it14, &b_usr_it14);
  // usertree->SetBranchAddress("min_chi2", &min_chi2, &b_min_chi2);
  // usertree->SetBranchAddress("min_sin2_theta_24_val", &min_sin2_theta_24_val, &b_min_sin2_theta_24_val);

  // double array_t24[60+1][60+1] = {{0}};
  
  // int entries = usertree->GetEntries();
  // for(int ientry=0; ientry<entries; ientry++) {
  //   usertree->GetEntry(ientry);	
  ...
  for(int ic=0; ic<Ncontour; ic++) {
    ...
    for(int idx=0; idx<np_curv; idx++) {
      ...
      // int global_bin = h2_CL->FindBin(t14, m41);
      // int local_x, local_y, local_z;
      // h2_CL->GetBinXYZ(global_bin, local_x, local_y, local_z);
      // double user_value = array_t24[local_y][local_x];
      
      /// user convert from sin2_theta_14 to sin2_2theta_14, hhack
      //if( xx_value>0.5 ) continue;      
      //xx_value = 4*xx_value*(1-xx_value);

      /// user convert from sin2_theta_14 to sin2_2theta_ue
      //if( xx_value>0.5 ) continue;
      //xx_value = 4*xx_value*(1-xx_value) * 3e-3;
      ...
      //if( xx_value>=0.005 ) continue;/// hhack
      //if( xx_value>=0.0042) continue;
...
void TClass_CLs::plot_distribution(int idm2, int itheta, double dchi2, int index)
{
  ...
  if( 0 ) {/// hhack
    //cout<<" ---> check "<<dchi2<<"\t"<<lg_d4v<<"\t"<<lg_d3v<<endl;
  ...
  if( 0 ) {
    TLine *line_dchi2 = new TLine(dchi2, 0, dchi2, ymax_vv*1.2*0.8); line_dchi2->Draw();
    line_dchi2->SetLineColor(kBlack); line_dchi2->SetLineWidth(4);
  }
  ...
  //dm2_vv = 7.3;
  //theta_vv = 0.38;
  //lg->SetHeader(TString::Format("#splitline{#Deltam^{2}_{41} = %4.2f eV^{2}}{sin^{2}2#theta_{14} = %5.2f}", dm2_vv, theta_vv));
...
void ea_frequentis_CLs()
{
  ...
  roofile_data = "./ub_new_numuDISonly_CLs/sub_sm_CLs_000001_data_uuONLY_tuned_numuDataRatio.root";
  ...
  //test_CLs->plot_distribution(38, 46, 4.80649, 1);
  //test_CLs->plot_distribution(30, 50, 7.5830893, 2);
  //test_CLs->plot_distribution(30, 44, 2.1703495, 2);
  ...
  //test_CLs->func_quantiles(0.5, 10, 1);
